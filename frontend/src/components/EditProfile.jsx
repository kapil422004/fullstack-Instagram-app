...
const EditProfile = () => {
  ...
  const editProfileHandler = async () => {
    ...
    try {
      setLoading(true);
      const res = await axios.post(backendUserUrl + "/edit-profile", formData, {
        headers: {
      ...
      <section className="flex flex-col gap-6 w-full my-8">
        ...
        <div className="flex items-center justify-between bg-gray-100 rounded-xl p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-bold text-sm">{authUser?.userName}</h1>
              <span className="text-gray-600">{authUser?.bio || ""}</span>
            </div>
          </div>
          <input
            ref={imageRef}
